...
def export_Hiddify(t_ips, f_ips):
    ...
    config_prefix = f'warp://{t_ips[0]}?ifp=10-20&ifps=20-60&ifpd=5-10#𓄂𓆃🇮🇷IR&&detour=warp://{t_ips[1]}?ifp=10-20&ifps=20-60&ifpd=5-10#WoW🇩🇪'ÐΛɌ₭ᑎΞ𐒡𐒡

    title = "//profile-title: base64:" + base64.b64encode('〘⬳𓄂𓆃⟿〙'.encode('utf-8')).decode(
        'utf-8') + "\n"
...
def export_SingBox(t_ips, arch):
    with open('SingBox Temp/temp.json', 'r') as f:
        data = json.load(f)
    ...
    main_wg = toSingBox('WARP🇮🇷𓄂𓆃', t_ips[0], "direct")
    data["outbounds"].insert(1, main_wg)
    wiw_wg = toSingBox('WARP🇩🇪'ÐΛɌ₭ᑎΞ𐒡𐒡, t_ips[1], "WARP-MAIN")
